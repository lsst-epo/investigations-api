...
class AssessmentsController extends Controller
{
    ...
    /** Volume handle to upload into. Defaults to self::VOLUME. */
    ...
    public function actionUploadAssets(): int
    {
        ...
        $handle = $this->volume ?: self::VOLUME;
        $volume = Craft::$app->getVolumes()->getVolumeByHandle($handle);
        if (!$volume) {
            $this->stderr("Volume '$handle' not found.\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $fs = $volume->getFs();
        $this->stdout(sprintf(
            "Uploading into '%s' (%s, %s).\n",
            $volume->name,
            $fs->name,
            $fs instanceof \craft\fs\Local
                ? 'local: ' . $fs->getRootPath()
                : get_class($fs)
        ), $this->volume ? Console::FG_YELLOW : Console::FG_GREY);

        $folder = Craft::$app->getAssets()->getRootFolderByVolumeId($volume->id);
        $priorMap = array_map('intval', $this->readJson($dir . '/asset-map.json'));
        $map = [];
        $created = 0;
        $reused = 0;
